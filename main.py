...
class BaseScreen(MDScreen):
    # Родоначальник всех экранов. Создает адаптивную подложку и управляет модальным вводом.
    ...
    def on_enter(self):
        """Автоматическое событие Kivy: срабатывает КАЖДЫЙ РАЗ, когда инженер 
        физически заходит на любой экран приложения. Мгновенно активирует сканер полей!"""
        self.auto_bind_textfields()

    def auto_bind_textfields(self):
        """Бэкенд-метод: находит абсолютно все MDTextField на текущем экране и привязывает фокус"""
        from kivymd.uix.textfield import MDTextField
        for child in self.walk():
            if isinstance(child, MDTextField):
                # Сначала принудительно отвязываем старый фокус (чтобы не было дублирования при повторном входе)
                child.unbind(focus=self.on_field_focus)
                # Намертво привязываем событие фокуса к нашему изолированному методу
                child.bind(focus=self.on_field_focus)

    def on_field_focus(self, instance, value):
        """Инженерный перехват фокуса: срабатывает только на конкретном нажатом поле,
        полностью исключая баг зацикливания и двоения цифр!"""
        if value:  # Если поле получило курсор (фокус == True)
            hint = instance.hint_text if instance.hint_text else "ввод данных"
            self.create_input_modal(hint, instance)
    ...
    def create_input_modal(self, field_title, target_field):
        """Создает изолированное модальное окно в верхней половине экрана над клавиатурой."""
        ...
        modal = ModalView(
            size_hint=(1.0, 0.55),
            pos_hint={'top': 1.0},
            auto_dismiss=True,  # Закрытие кликом в любую пустую зону сзади
        ...
        lbl_hint = MDLabel(
            text=f"ВВОД ДАННЫХ: {str(field_title).upper()}",
            font_style="Button",
            bold=True,
        ...
        # Создаем большое поле ввода и сразу подтягиваем туда текущий текст маленького поля
        modal_input = MDTextField(
            text=str(target_field.text),
            font_size="24sp",
            mode="rectangle",
            line_color_focus=[0.9, 0.4, 0.0, 1],
            size_hint_y=None,
            height=dp(54),
            focus=True  # Клавиатура сама мгновенно откроется в этой строке
        )

        # ТЕХНОЛОГИЧЕСКАЯ СВЯЗКА (Слушатели текста):
        # Всё, что пишется в большой строке, мгновенно дублируется в маленькую, и наоборот!
        def sync_modal_to_target(instance_input, text_value):
            target_field.text = text_value

        def sync_target_to_modal(instance_target, text_value):
            modal_input.text = text_value

        # Намертво привязываем двусторонний слушатель текста
        modal_input.bind(text=sync_modal_to_target)
        target_field.bind(text=sync_target_to_modal)

        # Функция, которая сработает при закрытии окна (нажатии ОК или клике мимо)
        def on_dismiss_modal(instance):
            # Отвязываем слушателей, чтобы освободить оперативную память устройства
            modal_input.unbind(text=sync_modal_to_target)
            target_field.unbind(text=sync_target_to_modal)
            # Безопасно снимаем фокус с маленького поля ввода на основном экране
            target_field.focus = False

        modal.bind(on_dismiss=on_dismiss_modal)
        ...
        layout.add_widget(lbl_hint)
        layout.add_widget(modal_input)
...
class WellViewScreen(BaseScreen):   # Экран визуализации. «Сцена», на которой запускается анимация закачки раствора.
    def __init__(self, **kw):
        ...
        btn_repeat = MDFillRoundFlatButton(
            text="ПОВТОРИТЬ", size_hint_x=1, height=dp(50),
...
class CalcScreen(BaseScreen): # Экран технологических расчетов. Наследует BaseScreen, получая его адаптивный фон и фикс клавиатуры.
    def __init__(self, **kw):
        ...
        self.container.add_widget(MDLabel(text="КАТЕГОРИЯ СКВАЖИНЫ :", font_style="Caption", theme_text_color="Hint")) 
        ...
        self.btn_nkt = MDFillRoundFlatButton(text="ВЫБРАТЬ ТИП НКТ", size_hint=(1, 1), md_bg_color=(0.1, 0.3, 0.6, 1), on_release=self.open_nkt_menu) 
        ...
        self.f_h_tvd = self.create_small_field("Глубина скв. по вертикали (м)", "2500") # Истинная глубина скважины по вертикали (для гидростатики)
        self.f_h_perf = self.create_small_field("Интервал перфорации (кровля, м)", "2700") # Верхняя граница вскрытого пласта
        ...
        self.f_rho = self.create_small_field("Текущая плотность раствора в скв. (г/см³)", "1.12") # Удельный вес глушильной жидкости в стволе
    ...
    def refresh_ui_elements(self):
        ...
        if "ЭЦН" in self.selected_gno.upper(): 
            ...
            self.f_l60 = self.create_small_field("Длина НКТ 60 (м)", "1500") 
            ...
            self.f_h_v = self.create_small_field("Глубина подвески ГНО / Воронки (м)", "2500") 
        ...
        if "ЭЦН" not in self.selected_gno.upper() and self.v_pipe is None: 
            ...
            attention_text = MDLabel( 
                text="[b]ВНИМАНИЕ![/b] СНАЧАЛА ВЫБЕРИТЕ ТИП НКТ!", 
    ...
    def do_calc(self, *args):
        ...
        try:
            # =====================================================================
            # БЭКЕНД-ПОДГОТОВКА: ДИНАМИЧЕСКОЕ ЧТЕНИЕ КОЭФФИЦИЕНТОВ ИЗ SETTINGS.JSON
            # Загружаем калибруемые инженером Кз из файла конфигурации настроек
            # =====================================================================
            from kivy.storage.jsonstore import JsonStore
            import os
            
            store = JsonStore(os.path.join(BASE_DIR, 'settings.json'))
            kz1_val, kz2_val, kz3_val = 1.10, 1.07, 1.05
            
            if store.exists('engineering_specs'):
                specs = store.get('engineering_specs')
                kz1_val = float(specs.get('kz1', 1.10))
                kz2_val = float(specs.get('kz2', 1.07))
                kz3_val = float(specs.get('kz3', 1.05))
            ...
            if p_pl == 0:
                p_izb_calc = p_ust if p_ust > 0 else p_zatr  # Берем наибольшее избыточное давление на устье
                # Формула: Избыточное давление на устье + Гидростатика текущего столба жидкости ((Глубина * Плотность) / 10)
            ...
            # Выбор коэффициента запаса Кз согласно требованиям Ростехнадзора на основе динамических настроек JSON
            if "1 КАТЕГОРИЯ" in self.well_category:
                k_zapas = kz1_val  # Настройки Кз 1-й категории (Газ)
            elif "2 КАТЕГОРИЯ" in self.well_category:
                k_zapas = kz2_val  # Настройки Кз 2-й категории (Нефть)
            else:
                k_zapas = kz3_val  # Настройки Кз 3-й категории (Обычные)
            ...
            if self.kill_method == "STAND":
                ...
                # Другие методы (Бурильщика, Задавка): берем избыток на устье
                p_izb = p_ust if p_ust > 0 else p_zatr  
                # ИСПРАВЛЕНО: Применяем живой коэффициент k_zapas к давлению репрессии, чтобы плотность задавки 
                # для 1-й категории гарантировала 10% технологический запас Ростехнадзора на устье!
                rho = round(rho_c + ((p_izb * 10.197 / h_ref) * k_zapas) + 0.05, 2)
            ...
            # ИСПРАВЛЕНО: Меняем делитель с 1 000 000 на 10 000, чтобы перевести см² площади колонны в кубические метры!
            # Аварийный минус полностью ликвидирован, объем кольцевого пространства примет верное положительное значение
            ...
            # =====================================================================
            # БЭКЕНД-ФИКС: ДИНАМИЧЕСКОЕ ЧТЕНИЕ АВАРИЙНОГО ЗАПАСА ИЗ SETTINGS.JSON
            # Убираем жесткий хардкод +3.0 м³, теперь объем мерника управляется из настроек!
            # =====================================================================
            v_zapas_val = 3.0  # Заводской дефолт по регламенту КРС
            if store.exists('engineering_specs'):
                specs = store.get('engineering_specs')
                v_zapas_val = float(specs.get('v_zapas', 3.0))
            ...
            # ИСПРАВЛЕНО: Вместо жесткого хардкода +3.0 прибавляем калибруемый из настроек объем мерника
            v_total_with_zap = round(v_total + v_zapas_val, 2)
            
            # ВЫЧИСЛЯЕМ ЖИДКОСТЬ ГЛУШЕНИЯ ДЛЯ ЦИРКУЛЯЦИИ (Трубы + Затрубное + Мерник из настроек)
            v_kill_solution = v_trub + v_zatrub + v_zapas_val
            
            # ИСПРАВЛЕНО: Время закачки ЦА-320 теперь считается строго от ЧИСТОГО ОБЪЕМА ЖИДКОСТИ ГЛУШЕНИЯ (47.00 м³)!
            t_min = (v_kill_solution * 1000) / Q if Q > 0 else 0
            ...
            if "ЭЦН" in self.selected_gno.upper():
                ecn_prefix = (
                    ...
                    f"• Перед началом любых технологических операций глушения сбросьте металлический ломик (бросавку) в трубное空间 НКТ.\n"
            ...
            # =====================================================================
            # БЭКЕНД-ФИКС: ДИНАМИЧЕСКОЕ ЧТЕНИЕ ДОПУСКА СПО ИЗ SETTINGS.JSON
            # Убираем жесткий хардкод 0.5 м³, теперь порог тревоги управляется из настроек!
            # =====================================================================
            v_crit_spo_val = 0.5  # Заводской дефолт порога чувствительности
            if store.exists('engineering_specs'):
                specs = store.get('engineering_specs')
                v_crit_spo_val = float(specs.get('v_crit_spo', 0.5))
            ...
            if self.kill_method == "STAND":
                ...
                if "ПАКЕР (СНЯТ)" in self.selected_gno.upper():
                    ...
                    instr = ecn_prefix + (
                        ...
                        f"• [b]Суть метода:[/b] Замещение жидкости в скважине новым раствором глушения плотностью [b]{rho:.2f} г/см³[/b] в объеме [b]{v_total_with_zap:.2f} м³[/b].\n"
                        ...
                        # ИСПРАВЛЕНО: Вместо жесткого текста '0.5 м³' подставляем живую переменную из Настроек!
                        f"• [b]Контроль параметров:[/b] Каждые 15 минут контролируйте объемы долива и затруба по мерникам ЦС. При разнице затруба и долива > [b]{v_crit_spo_val:.1f} м³[/b] — немедленно СТОП СПО и герметизация устья превентором!\n"
                    ...
                    instr = ecn_prefix + (
                        ...
                        f"• [b]Технология закачки:[/b] Закачка ведется через трубное пространство. Обеспечьте плавный пуск насоса ЦА-320 на {self.gear}-й передаче при {self.rpm} об/мин.\n"
                ...
                instr = ecn_prefix + (
                    ...
                    f"• Запустите ЦА-320 и начните вымыв пачки флюида на поверхность через дроссель. Газ начнёт выходить на устье из затруба практически сразу.\n"
                    # ИСПРАВЛЕНО: Указали реальную физику вымыва пачки по затрубу вместо объема труб v_trub!
                    f"• Регулированием штуцера поддерживайте давление в бурильных трубах (НКТ) ПОСТОЯННЫМ. Первый цикл и полный вымыв притока завершатся после закачки объема затруба [b]{v_zatrub:.2f} м³[/b].\n\n"
                ...
                instr = ecn_prefix + (
                    ...
                    f"[b]ЭТАПЫ ПРОВЕДЕНИ ПРИГОТОВЛЕНИЯ СОГЛАСНО РЕГЛАМЕНТУ IWCF:[/b]\n"
                    f"1. [b]Ожидание и приготовление:[/b] Скважина находится в закрытом состоянии под давлением. В это время на буровой готовится утяжеленный раствор проектной плотности [b]{rho:.2f} г/см³[/b] в объеме [b]{v_kill_solution:.2f} м³[/b].\n"
                ...
                # Метод 4: ЗАДАВКА В ПЛАСТ (BULLHEADING — ЕСЛИ ПАКЕР НЕ УСТАНОВЛЕН)
                # ИСПРАВЛЕНО: Указали бурильщику качать строго объём кольцевого пространства v_zatrub, 
                # чтобы не перелить лишний раствор в закрытые трубы НКТ и не порвать пласт!
                instr = (
                    f"[color=#FF5555][b]ТЕХНОЛОГИЧЕСКАЯ КАРТА ЗАДАВКИ (БЕЗ ПАКЕРА)[/b][/color]\n"
                    f"• [b]Суть метода:[/b] Глушение давлением без циркуляции путём продавливания притока обратно в пласт.\n"
                    f"• [b]Технология ведения работ:[/b] Подключите ЦА-320 к затрубному пространству. Закачивайте раствор плотностью [b]{rho:.2f} г/см³[/b].\n"
                    f"• [b]Объём закачки:[/b] Качайте объём кольцевого пространства [b]{v_zatrub:.2f} м³[/b] напрямую в затруб. Трубное пространство (НКТ) должно быть закрыто!\n"
                    f"• [b]Внимание:[/b] Ведите непрерывный контроль устьевого давления, не превышайте порог гидроразрыва пласта (Р_гр)!"
                )
            ...
            res_scr.selected_gno = self.selected_gno  # Передаем текущий тип ГНО для вывода на экран результатов
            ...
            # ВЫЧИСЛЯЕМ ТЕХНОЛОГИЧЕСКИЕ ПЕРЕМЕННЫЕ ДЛЯ РАЗДЕЛЬНОГО ВЫВОДА ОБЪЕМОВ ПО ТРЕБОВАНИЮ ИНЖЕНЕРА:
            # 1. Полная геометрическая вместимость ствола до искусственного забоя ЦКОД (м³)
            v_skv_to_ckod = v_trub + v_zatrub + v_pod
            ...
            res_scr.result_text.text = (
                ...
                f"• Общий объем скважины до ЦКОД: [b]{v_skv_to_ckod:.2f} м³[/b]\n"  # Вместимость ствола до забоя
                f"• Объем жидкости глушения (+{v_zapas_val:.1f}м³): [color=#33FF33][b]{v_kill_solution:.2f} м³[/b][/color]\n"  # Объем раствора для заказа
                f"• Время закачки агрегатом: [color=#FFFF33][b]{int(t_min)} мин[/b][/color]\n"  # Поминутное время работы насоса ЦА-320
                # Распределение объемов: внутренний объем НКТ и объем внешнего затрубного пространства ствола (БЕЗ слова кольцевое)
                f"• Инструмент НКТ: [b]{v_trub:.2f} м³[/b] | Затрубное: [b]{v_zatrub:.2f} м³[/b]"
            )
...
class SpoScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        self.res_label = MDLabel(
            text="[color=#AAAAAA]Выберите тип НКТ[/color]", 
            theme_text_color="Custom", text_color=(1, 1, 1, 1),
    ...
    def calculate_doliv(self, *args):
        try:
            ...
            # 1. БЭКЕНД-ПОДХВАТ: Динамическое чтение допуска СПО из настроек JSON
            from kivy.storage.jsonstore import JsonStore
            import os
            
            store = JsonStore(os.path.join(BASE_DIR, 'settings.json'))
            v_crit_spo_val = 0.5  # Заводской дефолт порога
            
            if store.exists('engineering_specs'):
                specs = store.get('engineering_specs')
                v_crit_spo_val = float(specs.get('v_crit_spo', 0.5))
            
            # 2. ШТАТНЫЙ РАСЧЕТ ОБЪЕМА В ЛИТРАХ
            v_factor = (self.v_metalla + self.v_vnutr) if self.siphon_mode else self.v_metalla
            vol = (cnt * l_one) * v_factor * 1000
            
            # Заполняем текстовое поле результата: чистый литраж + строгое предупреждение по кнопке настроек
            self.res_label.text = (
                f"[size=24sp][color=#33FF33]ДОЛИВ: {vol:.1f} л[/color][/size]\n"
                f"[size=14sp][color=#FF5555]При разнице объёмов более [b]{v_crit_spo_val:.1f} м³[/b] —  СТОП СПО , загерметизировать устье скважины ![/color][/size]"
            )
        except Exception:
            self.res_label.text = "Ошибка"
...
class FishingScreen(BaseScreen):
    ...
    def draw_calc_ui(self):
        ...
        help_t = (
            ...
            "• Дай натяжку выше веса на (10-15 тн).\n"
            "• Замерь вытяжку трубы в сантиметрах."
        )
        ...
        self.f_force = MDTextField(hint_text="Натяжка (тонн)", mode="fill", input_filter="float")
        self.f_stretch = MDTextField(hint_text="Вытяжка (сантиметры)", mode="fill", input_filter="float")
...
<OpenBrigadesScreen>:
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            MDLabel:
                text: "ЗАРЕГИСТРИРОВАННЫЕ БРИГАДЫ:"
                font_style: "Caption"
    ...
    MDBoxLayout:
        ...
        ScrollView:
            ...
            MDBoxLayout:
                ...
                MDTextField:
                    id: f_br_phone_int
                    hint_text: "Внутренний стационарный телефон (АТС)"
                    mode: "fill"
                    size_hint_y: None
                    height: "44dp"
                    input_type: "number"
                ...
                MDTextField:
                    id: f_br_phone_mob
                    hint_text: "Сотовый телефон мастера"
                    mode: "fill"
                    size_hint_y: None
                    height: "44dp"
                    input_type: "number"
...
class SettingsScreen(BaseScreen): # Экран глобальных настроек приложения WELL CONTROL. Наследует BaseScreen для получения адаптивного фона.
    def __init__(self, **kw):
        ...
        # ИСПРАВЛЕНО: Заменяем американскую ссылку GitHub на официальный адрес вашего приложения в RuStore,
        # чтобы софт на 100% успешно прошёл модерацию, а кнопка обновления вела прямо в маркет!
        self.update_url = "https://rustore.ru"
        ...
        app = MDApp.get_running_app()
        current_scale = getattr(app, 'menu_scale', 1.5)
...
class CementAcidScreen(BaseScreen):
    def __init__(self, **kw):
        ...
        self.toolbar.add_widget(btn_back)
        self.toolbar.add_widget(MDLabel(text="ЦМОСТ / ВАННЫ", theme_text_color="Custom", text_color=(1,1,1,1), font_style="H6"))
        main_layout.add_widget(self.toolbar)
        ...
        tab_grid = MDGridLayout(cols=2, spacing=dp(5), size_hint_y=None, height=dp(45))
        self.b1 = MDFillRoundFlatButton(text="ЦЕМЕНТ", size_hint=(1,1))
        self.b2 = MDFillRoundFlatButton(text="ВАННА", size_hint=(1,1))
        
        # Безопасная привязка переключателей без двойных лямбд
        self.b1.bind(on_release=lambda x: self.switch_mode("CEMENT"))
        self.b2.bind(on_release=lambda x: self.switch_mode("ACID"))
        
        tab_grid.add_widget(self.b1)
        tab_grid.add_widget(self.b2)
        self.container.add_widget(tab_grid)
        
        # ПОЛЯ ВВОДА И БЭКЕНД-АВТОМАТИКА
        self.inputs_box = MDBoxLayout(orientation='vertical', adaptive_height=True, spacing=dp(8))
        
        # Внутренний локальный хелпер создания текстовых полей
        def create_f(hint, default):
            from kivymd.uix.textfield import MDTextField
            return MDTextField(hint_text=hint, text=default, mode="fill", input_filter="float")

        self.f_diam = create_f("Ø экспл. колонны (мм)", "168")
        self.f_depth = create_f("Глубина воронки (м)", "1550")
        self.f_target = create_f("Нижний интервал установки / Подошва моста (м)", "1550")
        self.f_vol = create_f("Объем смеси (м³)", "1.5")

        # Безопасное стартовое состояние: воронка равна интервалу и заблокирована
        self.f_depth.disabled = True

        # БЭКЕНД-СШИВКА: Заставляем воронку зеркально повторять ввод за полем интервала установки
        self.f_target.bind(text=self.on_target_text_change)

        # ОРИГИНАЛЬНАЯ СИНЯЯ КНОПКА ВЫБОРА НКТ (КАК В ДРУГИХ ТВОИХ КЛАССАХ)
        self.f_nkt_btn = MDFillRoundFlatButton(
            text="ВЫБРАТЬ ТИП НКТ", 
            size_hint=(1, None), 
            height=dp(48), 
            md_bg_color=(0.1, 0.3, 0.6, 1), 
            on_release=self.open_nkt_menu  # Вызывает метод self ниже
        )

        # Укладываем синю кнопку НКТ и текстовые поля в inputs_box
        for f in [self.f_nkt_btn, self.f_diam, self.f_depth, self.f_target, self.f_vol]:
            self.inputs_box.add_widget(f)
        self.container.add_widget(self.inputs_box)

        # КНОПКА РАСЧЕТА ТЕХНОЛОГИИ
        btn_calc = MDFillRoundFlatButton(
            text="РАССЧИТАТЬ ТЕХНОЛОГИЮ",
            size_hint=(1, None),
            height=dp(48),
            md_bg_color=(0.1, 0.6, 0.2, 1),
            on_release=self.do_calc
        )
        self.container.add_widget(btn_calc)

        # ТЕКСТОВОЕ ПОЛЕ ВЫВОДА РЕЗУЛЬТАТОВ / ИНСТРУКЦИИ
        self.res_label = MDLabel(
            text="[color=#AAAAAA]Введите параметры и нажмите расчёт[/color]",
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1),
            font_style="Body1",
            size_hint_y=None,
        ...
        # ЖЕСТКИЙ ИНДЕКС ВЫСОТЫ [1]: Теперь парсер его не сожрёт!
        self.res_label.bind(texture_size=lambda ins, sz: setattr(ins, 'height', sz[1] + dp(20)))
        self.container.add_widget(self.res_label)

        # ФИНАЛЬНАЯ СБОРКА И СШИВКА СЛОЕВ ЭКРАНА
        scroll.add_widget(self.container)
        main_layout.add_widget(scroll)
        self.add_widget(main_layout)
    ...
    def set_nkt(self, r):
        ...
        try:
            ...
            # Меняем текст на нашей синей кнопке, показывая бурильщику выбранную трубу
            if hasattr(self, 'f_nkt_btn'):
                self.f_nkt_btn.text = f"ВЫБРАНО: НКТ {self.tech_nkt_name}"
    ...
    def on_target_text_change(self, instance, value):
        """Зеркальный бэкенд-мост: копирует интервал в глубину воронки только для Цемента"""
        if hasattr(self, 'mode') and self.mode == "CEMENT" and hasattr(self, 'f_depth'):
            self.f_depth.text = value

    def switch_mode(self, mode_name):
        """Инженерное разделение интерфейса с жёстким контрастным переключением вкладок"""
        self.mode = mode_name
        self.res_label.text = "[color=#AAAAAA]Введите параметры и нажмите расчёт[/color]"
        
        # ИСПРАВЛЕНО ОКОНЧАТЕЛЬНО: Активная вкладка горит сигнальным КРАСНЫМ, неактивная тускнеет в серый!
        # Цвета заданы в формате RGBA: Красный (0.8, 0.1, 0.1, 1) | Тёмно-серый (0.2, 0.2, 0.2, 0.6)
        self.b1.md_bg_color = (0.8, 0.1, 0.1, 1) if mode_name == "CEMENT" else (0.2, 0.2, 0.2, 0.6)
        self.b2.md_bg_color = (0.8, 0.1, 0.1, 1) if mode_name == "ACID" else (0.2, 0.2, 0.2, 0.6)
        
        if mode_name == "CEMENT":
            if hasattr(self, 'f_target'):
                self.f_target.hint_text = "Нижний интервал установки / Подошва моста (м)"
            if hasattr(self, 'f_depth') and hasattr(self, 'f_target'):
                self.f_depth.text = self.f_target.text
                self.f_depth.disabled = True
        else:
            if hasattr(self, 'f_target'):
                self.f_target.hint_text = "Интервал установки / Кровля перфорации (м)"
            if hasattr(self, 'f_depth'):
                self.f_depth.disabled = False
    def do_calc(self, *args):
        try:
            # ИНЖЕНЕРНАЯ ЗАЩИТА: Проверяем, выбрал ли бурильщик трубу НКТ из базы данных Excel
            if not hasattr(self, 'v_pipe') or self.v_pipe is None or self.v_pipe == 0.0:
                self.res_label.text = "[color=#FFCC00][b]ВНИМАНИЕ:[/b] Сначала нажмите «ВЫБРАТЬ ТИП НКТ»"
                return

            def val(f): 
                return float(f.text or 0)
                
            D, Hv, H_tar, V_sm = val(self.f_diam), val(self.f_depth), val(self.f_target), val(self.f_vol)
            
            # 1. Точный расчёт вместимости 1 погонного метра колонны (л/м)
            v_col_litr = ((D - 15) ** 2) * 0.0007854 / 10 
            
            # 2. Общий внутренний объём НКТ до самой воронки (в ЛИТРАХ)
            v_nkt_total_litr = self.v_pipe * Hv 
            
            # 3. Высота столба технологической смеси (цемента или кислоты) в затрубном пространстве колонны (м)
            h_sm_col = (V_sm * 1000) / v_col_litr 
            
            # 4. Разница глубин между воронкой и интервалом установки (м)
            h_diff = max(0, H_tar - Hv)
            
            if self.mode == "CEMENT":
                # Расчёт мешков цемента ПЦТ-I-50 по плотности раствора (~1.85 г/см³)
                bags = int(V_sm * 23)
                
                # ИСПРАВЛЕНО: Корректный метод уравновешенных столбов (все вычисления строго в ЛИТРАХ)
                v_prodavka_cement_litr = v_nkt_total_litr - (h_sm_col * self.v_pipe)
                v_prodavka_cement_litr = max(100.0, min(v_prodavka_cement_litr, v_nkt_total_litr))
                
                res = (
                    f"[size=18sp][color=#33FF33]ИНСТРУКЦИЯ: ЦЕМЕНТНЫЙ МОСТ[/color][/size]\n"
                    f"[color=#55FFFF]Выбрано: {getattr(self, 'tech_nkt_name', 'НКТ')} (вмещаемость: {self.v_pipe:.4f} л/м)[/color]\n\n"
                    f"1. [b]Закачка:[/b] Закачай в трубы [b]{V_sm} м³[/b] цементного раствора.\n"
                    f"   (Для этого потребуется примерно [b]{bags} мешков[/b] цемента по 50кг).\n"
                    f"2. [b]Продавка:[/b] Качай следом продавочный раствор в объёме [b]{v_prodavka_cement_litr / 1000:.2f} м³[/b].\n"
                    f"   [i]*Расчёт по методу уравновешенных столбов с учётом гидростатики затруба.*[/i]\n"
                    f"3. [b]Подъем:[/b] Сразу после продавки приподними воронку НКТ выше [b]{H_tar - h_sm_col - 20:.0f} м[/b].\n"
                    f"4. [b]Срезка:[/b] Включи обратную промывку (качай в затруб), чтобы чисто вымыть излишки цемента."
                )
            else:
                # ИСПРАВЛЕНО: Объём доводки до воронки в кубических метрах (Объём НКТ минус Объём Кислоты)
                v_dovodka_acid_m3 = (v_nkt_total_litr / 1000) - V_sm
                v_dovodka_acid_m3 = max(0.0, v_dovodka_acid_m3)
                
                # ИСПРАВЛЕНО: Точный объём выталкивания кислоты из-под воронки в интервал перфорации
                v_prodavka_acid_m3 = V_sm + ((v_col_litr * h_diff) / 1000)
                
                res = (
                    f"[size=18sp][color=#33FF33]ИНСТРУКЦИЯ: КИСЛОТНАЯ ВАННА[/color][/size]\n"
                    f"[color=#55FFFF]Выбрано: {getattr(self, 'tech_nkt_name', 'НКТ')} (вмещаемость: {self.v_pipe:.4f} л/м)[/color]\n\n"
                    f"1. [b]Закачка:[/b] Закачай в трубы [b]{V_sm} м³[/b] кислотного состава.\n"
                    f"2. [b]Доводка:[/b] Качай раствор в трубы в объёме [b]{v_dovodka_acid_m3:.2f} м³[/b].\n"
                    f"   [b]ЗАТРУБ ДОЛЖЕН БЫТЬ ОТКРЫТ![/b] (Контролируй выход затрубной жидкости).\n"
                    f"3. [b]Продавка:[/b] Как только докачал объём доводки, [color=#FF5555][b]ЗАКРОЙ ЗАТРУБ![/b][/color]\n"
                    f"4. [b]Давим в пласт:[/b] Качай ещё [b]{v_prodavka_acid_m3:.2f} м³[/b] раствора в трубы.\n"
                    f"   (Кислота полностью выйдет из НКТ и продавится в интервал перфорации [b]{H_tar} м[/b]).\n"
                    f"5. [b]Финиш:[/b] Закрой центральную задвижку. Ожидание реакции (ОЗР) 2-4 часа."
                )
            self.res_label.text = res
        except Exception:
            self.res_label.text = "[color=#FF5555]ОШИБКА: Проверь, все ли цифры ввёл![/color]"
...
class KrsBookApp(MDApp): 
    ...
    def build(self): 
        ...
        if self.store.exists('menu_settings'):
            ...
            self.menu_scale = 1.5 # Базовый масштаб по умолчанию
    ...
    def on_start(self):
        """События при старте приложения: перехватывает скважину из Intent или буфера обмена."""
        shared_text = None

        # --- КАНАЛ 1: Нативный перехват Android Intent (через кнопку Поделиться) ---
        try:
        ...
        # --- КАНАЛ 2: Перехват через Буфер Обмена (если текст просто СКОПИРОВАН в МАКСе) ---
        if not shared_text:
            try:
                from kivy.core.clipboard import Clipboard
                clipboard_content = Clipboard.paste()
                if clipboard_content and "WELL_DATA_SHARE:" in clipboard_content:
                    shared_text = clipboard_content
                    Clipboard.copy("") # Чистим буфер после успешного перехвата, чтобы не дублировать
            except Exception:
                pass

        # --- ЕДИНЫЙ ПАРСЕР ДАННЫХ (Для обоих каналов) ---
        if shared_text and "WELL_DATA_SHARE:" in shared_text:
            try:
                clean_data = shared_text.replace("WELL_DATA_SHARE:", "").strip()
                p = clean_data.split('|')
                
                if len(p) >= 17:
                    field, pad, well = str(p[0]), str(p[1]), str(p[2])
                    p_buf, p_zatr, p_pl = str(p[3]), str(p[4]), str(p[5])
                    rho_c, depth, l_hvost = str(p[6]), str(p[7]), str(p[8])
                    perf_top, perf_bot, d_ek = str(p[9]), str(p[10]), str(p[11])
                    nkt_type, h_gno, brigade = str(p[12]), str(p[13]), str(p[14])
                    operation, notes = str(p[15]), str(p[16])
                    
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute(
                        "INSERT INTO wells (field, pad, well, p_buf, p_zatr, p_pl, rho_c, depth, l_hvost, perf_top, perf_bot, d_ek, nkt_type, h_gno, brigade, operation, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (field, pad, well, p_buf, p_zatr, p_pl, rho_c, depth, l_hvost, perf_top, perf_bot, d_ek, nkt_type, h_gno, brigade, operation, notes)
                    )
                    ...
                    if hasattr(self, 'sm') and self.sm:
                        view_screen = self.sm.get_screen('view_well_screen')
                        view_screen.current_well_id = new_id
                        self.sm.current = 'view_well_screen'
            except Exception as e:
                print(f"Parser Error: {e}")

        # Стандартная привязка иконки и кнопки Назад
        if os.path.exists(ICON_PATH):
            self.icon = ICON_PATH
    ...
    def hook_keyboard(self, window, key, *args):
        ...
        if key == 27:
            ...
            # 1. Безопасно убираем экранную клавиатуру
            try:
                Window.release_all_keyboards()
            except Exception:
                pass
                
            # 2. Безопасно гасим ВСЕ открытые выпадающие списки (Popup/Spinner)
            popup_closed = False
            try:
            ...
            # 3. Безопасно забираем фокус со всех полей ввода через родной метод walk_widgets()
            try:
                for widget in list(self.walk_widgets()):
                    if hasattr(widget, 'focus') and widget.focus:
                        widget.focus = False
            except Exception:
                pass
            
            # Если мы только что закрыли всплывающее окно — остаёмся на этом же экране!
            if popup_closed:
                return True
            ...
            # НАВИГАЦИОННЫЙ ФИКС БЛОКНОТА, СПРАВОЧНИКОВ И ПЕРСОНАЛА:
                ...
                return True
            # Возвращаем True, чтобы остановить дальнейшую обработку клавиши (экранируем от закрытия)
        ...
        return True # Если нажата любая другая кнопка (буква/цифра) — просто пропускаем её без вылетов!
...
class CreateWellScreen(BaseScreen):
    def go_back(self):
...
class OpenWellScreen(MDScreen): # Класс экрана, отображающего весь список сохраненных в Блокноте скважин
    ...
    def on_enter(self):
        """Автоматическое событие Kivy: срабатывает при открытии списка.
        ТЕХНОЛОГИЧЕСКИЙ ФИКС: Откладываем тяжелую выгрузку из SQLite на 0.1 секунды.
        Это уберет долгий черный экран при разворачивании приложения из запущенных!"""
        from kivy.clock import Clock
        # Мгновенно возвращаем графическую оболочку на экран телефона, 
        # а чтение базы данных запускаем на следующем кадре
        Clock.schedule_once(lambda dt: self.lazy_load_wells(), 0.1)

    def lazy_load_wells(self):
        """Бэкенд-метод: изолированная тяжелая выгрузка карточек скважин из SQLite"""
...
class EditWellScreen(BaseScreen):# Класс экрана редактирования параметров существующей скважины, наследуемый от MDScreen
    ...
    def on_enter(self):
        """Автоматическое событие Kivy: безопасно подгружает параметры скважины в поля редактирования и через динамический поиск намертво заполняет калькулятор глушения давениями и НКТ!"""
        super().on_enter()
        ...
        cursor = conn.cursor()
        # Выгружаем строго все 17 столбцов, включая давления и тип НКТ
        ...
        if w:
            # 1. СТАНДАРТНЫЙ ПЕРЕНОС В КАРТОЧКУ РЕДАКТИРОВАНИЯ (ЧЕРЕЗ IDS)
            if 'e_field' in self.ids: self.ids.e_field.text = str(w[0]) if w[0] is not None else ""
            if 'e_pad' in self.ids: self.ids.e_pad.text = str(w[1]) if w[1] is not None else ""
            if 'e_well' in self.ids: self.ids.e_well.text = str(w[2]) if w[2] is not None else ""
            if 'e_p_buf' in self.ids: self.ids.e_p_buf.text = str(w[3]) if w[3] is not None else ""
            if 'e_p_zatr' in self.ids: self.ids.e_p_zatr.text = str(w[4]) if w[4] is not None else ""
            if 'e_p_pl' in self.ids: self.ids.e_p_pl.text = str(w[5]) if w[5] is not None else ""
            if 'e_rho_c' in self.ids: self.ids.e_rho_c.text = str(w[6]) if w[6] is not None else ""
            if 'e_depth' in self.ids: self.ids.e_depth.text = str(w[7]) if w[7] is not None else ""
            if 'e_l_hvost' in self.ids: self.ids.e_l_hvost.text = str(w[8]) if w[8] is not None else ""
            if 'e_perf_top' in self.ids: self.ids.e_perf_top.text = str(w[9]) if w[9] is not None else ""
            if 'e_perf_bot' in self.ids: self.ids.e_perf_bot.text = str(w[10]) if w[10] is not None else ""
            if 'e_d_ek' in self.ids: self.ids.e_d_ek.text = str(w[11]) if w[11] is not None else ""
            if 'e_nkt_drop' in self.ids: self.ids.e_nkt_drop.text = str(w[12]) if w[12] else "Выбрать НКТ"
            if 'e_h_gno' in self.ids: self.ids.e_h_gno.text = str(w[13]) if w[13] is not None else ""
            if 'e_operation' in self.ids: self.ids.e_operation.text = str(w[15]) if w[15] is not None else ""
            if 'e_notes' in self.ids: self.ids.e_notes.text = str(w[16]) if w[16] is not None else ""

            # =========================================================================
            # РОДНОЙ ИНЖЕНЕРНЫЙ ФИКС БРИГАДЫ: Подтягиваем мастера по сохранённому номеру w[14]
            if 'e_brigade' in self.ids:
                val = str(w[14]).strip() if w[14] is not None else ""
                if val and "мастер" not in val.lower():
                    try:
                        conn_b = get_db_connection()
                        cursor_b = conn_b.cursor()
                        cursor_b.execute("SELECT master, phone_int, phone_mob FROM brigades WHERE number=?", (val,))
                        b = cursor_b.fetchone()
                        conn_b.close()
                        if b:
                            master_name = b[0]
                            p_int = f", вн. {b[1]}" if b[1] else ""
                            p_mob = f", сот. {b[2]}" if b[2] else ""
                            self.ids.e_brigade.text = f"Бригада №{val} (мастер {master_name}{p_int}{p_mob})"
                        else:
                            self.ids.e_brigade.text = f"Бригада №{val}"
                    except Exception:
                        self.ids.e_brigade.text = val
                else:
                    self.ids.e_brigade.text = val
            # =========================================================================

            # =====================================================================
            # БЛОК 2: ТОТАЛЬНЫЙ ПЕРЕДАТЧИК ДАННЫХ В КАЛЬКУЛЯТОР РАСЧЕТОВ (CALCSCREEN)
            # =====================================================================
            if self.manager and 'calc_screen' in self.manager.screen_names:
                calc = self.manager.get_screen('calc_screen')
                if hasattr(calc, 'f_p_ust') and calc.f_p_ust: calc.f_p_ust.text = str(w[3]) if w[3] is not None else "0"
                if hasattr(calc, 'f_p_zatr') and calc.f_p_zatr: calc.f_p_zatr.text = str(w[4]) if w[4] is not None else "0"
                if hasattr(calc, 'f_p_pl') and calc.f_p_pl: calc.f_p_pl.text = str(w[5]) if w[5] is not None else ""
                if hasattr(calc, 'f_rho') and calc.f_rho: calc.f_rho.text = str(w[6]) if w[6] is not None else "1.12"
                if hasattr(calc, 'f_d_col') and calc.f_d_col: calc.f_d_col.text = str(w[11]) if w[11] is not None else "168"
                if hasattr(calc, 'f_ckod') and calc.f_ckod: calc.f_ckod.text = str(w[7]) if w[7] is not None else "3200"
                if hasattr(calc, 'f_l_hvost') and calc.f_l_hvost: calc.f_l_hvost.text = str(w[8]) if w[8] is not None else "0"
                if hasattr(calc, 'f_h_tvd') and calc.f_h_tvd: calc.f_h_tvd.text = str(w[13]) if w[13] is not None else "2500"
                if hasattr(calc, 'f_h_perf') and calc.f_h_perf: calc.f_h_perf.text = str(w[9]) if w[9] is not None else "2700"

            # 3. СИНХРОНИЗАЦИЯ ТИПА НКТ И ВЫЗОВ ПЕРЕРИСОВКИ ИНТЕРФЕЙСА
            if w[12]:
                calc.tech_nkt_name = str(w[12])
                if calc.v_pipe is None or calc.v_pipe == 0.0:
                    calc.v_pipe = 0.342
                calc._loading_db = True
                calc.refresh_ui_elements()
                if hasattr(calc, '_loading_db'):
                    delattr(calc, '_loading_db')
    ...
    def share_well(self):
        ...
        report = ( 
            f" РАБОТА ПО: Скв. №{self.ids.e_well.text} (К-{self.ids.e_pad.text})\n" # Номер скважины и кустовая площадка
...
class ViewWellScreen(MDScreen):
    ...
    def share_well(self):
        ...
        report = (
            f" СКВАЖИНА: №{self.ids.v_well.text} (К-{self.ids.v_pad.text})\n"
    ...
    def open_in_glushenie(self):
        """Технологический метод: экспортирует ТОЛЬКО ГЕОМЕТРИЮ И ДАВЛЕНИЯ скважины 
        в калькулятор ЦА-320, полностью исключая перенос НКТ и скрытые вылеты графики!"""
        calc_screen = self.manager.get_screen("calc")
        
        # 1. БЕЗОПАСНЫЙ ПЕРЕНОС ПАРАМЕТРОВ ДАВЛЕНИЙ И ТЕКУЩЕЙ ПЛОТНОСТИ
        if 'v_p_pl' in self.ids:
            calc_screen.f_p_pl.text = self.ids.v_p_pl.text       # Пластовое давление
            
        if hasattr(calc_screen, 'f_p_ust') and calc_screen.f_p_ust:
            # Проверяем все возможные варианты id буферного/трубного давления на вашем экране просмотра
            if 'v_p_buf' in self.ids: calc_screen.f_p_ust.text = self.ids.v_p_buf.text
            elif 'v_p_ust' in self.ids: calc_screen.f_p_ust.text = self.ids.v_p_ust.text
            elif 'p_buf' in self.ids: calc_screen.f_p_ust.text = self.ids.p_buf.text
            
        if hasattr(calc_screen, 'f_p_zatr') and calc_screen.f_p_zatr:
            # Проверяем все возможные варианты id затрубного давления на вашем экране просмотра
            if 'v_p_zatr' in self.ids: calc_screen.f_p_zatr.text = self.ids.v_p_zatr.text
            elif 'p_zatr' in self.ids: calc_screen.f_p_zatr.text = self.ids.p_zatr.text
            
        if 'v_rho_c' in self.ids:
            calc_screen.f_rho.text = self.ids.v_rho_c.text        # Текущая плотность раствора
        elif 'v_rho' in self.ids:
            calc_screen.f_rho.text = self.ids.v_rho.text
        
        # 2. СТАНДАРТНЫЙ ПЕРЕНОС ГЕОМЕТРИИ СКВАЖИНЫ
        if 'v_depth' in self.ids: calc_screen.f_ckod.text = self.ids.v_depth.text       # Искусственный забой
        if 'v_perf_top' in self.ids: calc_screen.f_h_perf.text = self.ids.v_perf_top.text  # Кровля перфорации
        if 'v_d_ek' in self.ids: calc_screen.f_d_col.text = self.ids.v_d_ek.text       # Диаметр ЭК
        
        if hasattr(calc_screen, 'f_l_hvost') and calc_screen.f_l_hvost:
            if 'v_l_hvost' in self.ids: calc_screen.f_l_hvost.text = self.ids.v_l_hvost.text # Длина хвостовика
            
        # 3. ВЫЗЫВАЕМ ШТАТНУЮ ПЕРЕРИСОВКУ ИНТЕРФЕЙСА (БЕЗ НКТ)
        calc_screen.refresh_ui_elements()
        
        # Переносим глубину подвески ГНО в динамически генерируемые поля (Воронка / ЭЦН)
            ...
            for child in calc_screen.gno_fields_box.children:
                if isinstance(child, MDTextField):
                    ...
                    if 'глубина' in hint or 'спуск' in hint or 'длина' in hint:
                        if 'v_h_gno' in self.ids:
                            child.text = self.ids.v_h_gno.text
                        break
                        
        # Финальный чистый переход инженера на работающий экран расчетов Глушения
        self.manager.current = "calc"
