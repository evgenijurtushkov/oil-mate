...
class BaseScreen(MDScreen): # Родоначальник всех экранов. Создает адаптивную подложку и управляет фокусом полей ввода.
    ...
    def _on_keyboard_height(self, window, height):
        ...
        Гарантирует видимость именно заполняемой ячейки без улетания верхних полей."""
        from kivy.metrics import dp
        from kivy.animation import Animation
        from kivy.uix.scrollview import ScrollView

        if not self.manager or self.manager.current != self.name:
            return  # Страховка: обрабатываем событие только на активном экране

        # 1. Автоматически находим ScrollView на текущем экране
        scroll_view = None
        ...
        if not scroll_view:
            return  # Если скролла нет, выходим

        # Сохраняем первоначальную высоту скроллера, если еще не сохранили
        if not hasattr(self, '_original_scroll_height'):
            self._original_scroll_height = scroll_view.height

        if height > 0:
            # --- СЦЕНАРИЙ: КЛАВИАТУРА ВЫЕХАЛА ---
            # Ищем, какое именно текстовое поле сейчас находится в фокусе
            focused_widget = None
            for child in self.walk():
            ...
            if focused_widget:
                # Вычисляем доступную высоту экрана (высота окна минус высота клавиатуры)
                available_height = window.height - height
                
                # Узнаем точную физическую координату Y нижнего края ячейки на экране телефона
                widget_y = focused_widget.to_window(0, 0)[1]

                # Если ячейка находится ниже кромки выехавшей клавиатуры или слишком близко к ней
                if widget_y < height + dp(20):
                    # Временно уменьшаем высоту зоны прокрутки, чтобы контент не залезал под клавиатуру
                    scroll_view.height = self._original_scroll_height - height
                    
                    # Мягко и автоматически докручиваем скроллер именно к активному полю ввода
                    # Функция padding задает комфортный зазор в 40dp над кнопками клавиатуры
                    scroll_view.scroll_to(focused_widget, padding=dp(40), animate=True)
        else:
            # --- СЦЕНАРИЙ: КЛАВИАТУРУ ЗАКРЫЛИ ---
            # Плавно возвращаем ScrollView его законную полную высоту
            scroll_view.height = self._original_scroll_height
            
            # Если ползунок скролла улетел за границы, мягко возвращаем его в рабочий диапазон
            if scroll_view.scroll_y < 0 or scroll_view.scroll_y > 1:
                target_y = max(0.0, min(1.0, scroll_view.scroll_y))
                Animation(scroll_y=target_y, d=0.1, t='out_quad').start(scroll_view)
