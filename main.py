...
from kivy.storage.jsonstore import JsonStore
from kivy.lang import Builder  # Добавили импорт Builder!
from kivymd.uix.boxlayout import MDBoxLayout
from kivymd.uix.floatlayout import MDFloatLayout  # Добавили эту строчку!
from kivy.utils import platform  # Добавили эту строчку!
...
class CalcScreen(BaseScreen):
    ...
    def do_calc(self, *args):
        ...
        try:
            ...
            # Давление и гидростатика рассчитываются по зоне перфорации
            ...
            # Геометрия и объемы рассчитываются строго до искусственного забоя L_ckod
            ...
            if "уст." in gno_low:
                ...
                v_total = v_zatrub + v_pod
            else:
                v_total = v_trub + v_zatrub + v_pod
    ...
    def menu_category(self, b):
        items = [
            {"viewclass": "OneLineListItem", "text": "1 КАТЕГОРИЯ: Газ; ГФ >= 200; АВПД; H2S", "theme_text_color": "Custom", "text_color": (1,1,1,1), "on_release": lambda: self.set_category_val("Скважина 1 категории!")},
            {"viewclass": "OneLineListItem", "text": "2 КАТЕГОРИЯ: Нефть; Рпл > Ргидр; H2S в ПДК", "theme_text_color": "Custom", "text_color": (1,1,1,1), "on_release": lambda: self.set_category_val("Скважина 2 категории!")},
            {"viewclass": "OneLineListItem", "text": "3 КАТЕГОРИЯ: Рпл <= Ргидр; H2S нет", "theme_text_color": "Custom", "text_color": (1,1,1,1), "on_release": lambda: self.set_category_val("Скважина 3 категории!")}
        ]
        ...
        self.m_cat = MDDropdownMenu(
            caller=b,
            items=items,
            width_mult=4.5, # Уменьшили ширину, чтобы меню не вылезало за экран
            position="bottom", # Меню откроется ровно под кнопкой
    ...
    def set_category_val(self, val):
        ...
        # Определяем короткое имя для аккуратного вывода на кнопку
        ...
        if hasattr(self, 'btn_category') and self.btn_category:
            self.btn_category.text = f"ОПАСНОСТЬ: {short_cat}"
...
class SettingsScreen(BaseScreen):
    ...
    def open_brigades_dialog(self, *args):
        ...
        list_scroll = ScrollView(size_hint_y=None, height=dp(200))
        ...
        if not rows:
            from kivymd.uix.label import MDLabel
            empty_lbl = MDLabel(
                text="Справочник пуст. Добавьте первую бригаду.",
                halign="center", theme_text_color="Hint",
                size_hint_y=None, height=dp(40), font_style="Body2"
            )
            self.brigades_list_container.add_widget(empty_lbl)
        else:
            # ТЕХНОЛОГИЧЕСКИЙ ФИКС: Корректно читаем кортеж базы по индексам [0], [1], [2], [3]
            for row in rows:
                b_num = str(row[0])
                b_master = str(row[1])
                p_int = f"вн. {row[2]}" if row[2] else ""
                p_mob = f"сот. {row[3]}" if row[3] else ""
                phones = " | ".join([p for p in [p_int, p_mob] if p])
                
                item = TwoLineListItem(
                    text=f"Бригада №{b_num} — мастер {b_master}",
                    secondary_text=phones if phones else "Контакты не указаны",
                    theme_text_color="Custom",
                    text_color=(1, 1, 1, 1),
                    secondary_theme_text_color="Custom",
                    secondary_text_color=(0.9, 0.4, 0.0, 1)
                )
                self.brigades_list_container.add_widget(item)
                
        list_scroll.add_widget(self.brigades_list_container)
        self.dialog_box.add_widget(list_scroll)
        ...
        add_btn = MDRaisedButton(
            text="➕   ДОБАВИТЬ НОВУЮ БРИГАДУ",
            md_bg_color=(0.1, 0.4, 0.8, 1),
            size_hint_x=1,
            on_release=self.open_add_brigade_fields
        )
        add_btn_anchor.add_widget(add_btn)
        self.dialog_box.add_widget(add_btn_anchor)
    ...
    def open_add_brigade_fields(self, *args):
        # Закрываем окно списка
        self.br_dialog.dismiss()
        
        # ТЕХНОЛОГИЧЕСКИЙ ФИКС: Облегченный режим "line" и короткие понятные подсказки
        self.br_num = MDTextField(
            hint_text="№ бригады", 
            mode="line", 
            size_hint_y=None, 
            height=dp(38)
        )
        self.br_master = MDTextField(
            hint_text="ФИО мастера ", 
            mode="line", 
            size_hint_y=None, 
            height=dp(38)
        )
        self.br_phone_int = MDTextField(
            hint_text="Внутренний телефон ", 
            mode="line", 
            size_hint_y=None, 
            height=dp(38), 
            input_filter='int'
        )
        self.br_phone_mob = MDTextField(
            hint_text="Сотовый телефон ", 
            mode="line", 
            size_hint_y=None, 
            height=dp(38), 
            input_filter='int'
        ...
        # Контейнер с увеличенными зазорами (spacing), чтобы поля железно не слипались
        add_content = MDBoxLayout(
            orientation="vertical",
            spacing=dp(16),
            adaptive_height=True,
            padding=[dp(10), dp(10), dp(10), dp(10)]
        )
        add_content.add_widget(self.br_num)
        add_content.add_widget(self.br_master)
        add_content.add_widget(self.br_phone_int)
        add_content.add_widget(self.br_phone_mob)

        # Открываем чистое и просторное окно ввода
        self.add_dialog = MDDialog(
            title="НОВАЯ КАРТОЧКА БРИГАДЫ",
            type="custom",
            content_cls=add_content,
            size_hint_x=0.9,
            auto_dismiss=False,
            buttons=[
                MDFlatButton(
                    text="ОТМЕНА", 
                    on_release=lambda x: self.cancel_add_brigade()
                ),
                MDRaisedButton(
                    text="СОХРАНИТЬ", 
                    md_bg_color=(0.1, 0.6, 0.2, 1), 
                    on_release=self.save_brigade_to_db
                )
            ]
        )
...
class MainMenuScreen(MDScreen):
    def __init__(self, **kw):
        ...
        self.subtitle_label = Label(
            text="«Твой расчет — твоя безопасность»", color=(1, 1, 1, 1),
        ...
        self.scroll = ScrollView(
            size_hint=(1, None),
            pos_hint={'center_x': 0.5, 'top': 0.82}, # Скролл красиво поджат к слогану
        ...
        c_blue = (0.1, 0.4, 0.8, 0.55)
        c_grey = (0.2, 0.25, 0.3, 0.55)

        # Ровно 8 рабочих кнопок
        grid_items = [
            ("ГЛУШЕНИЕ", "water-alert", c_blue, "calc"),
            ("КОНТРОЛЬ ДОЛИВА", "water-pump", c_blue, "spo"),
            ("ЦМОСТ / ВАННЫ", "format-color-fill", c_blue, "cmoist"),
            ("ЛОВИЛЬНЫЕ РАБОТЫ", "hook", c_blue, "fishing"),
            ("ИНЖЕНЕРНЫЙ ХАБ", "calculator", c_blue, "calc_hub"),
            ("ЖУРНАЛ СКВАЖИН", "notebook", c_blue, "notebook_main_menu"),
            ("ИНСТРУМЕНТ (СПРАВКА)", "book-open-variant", c_grey, "threads"),
            ("НАСТРОЙКИ ПРИЛОЖЕНИЯ", "cog", c_grey, "settings")
        ]

        for t, i, c, s in grid_items:
            self.menu_box.add_widget(MenuCard(t, i, c, s))

        self.scroll.add_widget(self.menu_box)
        ...
        self.root_ui.add_widget(self.scroll)
    ...
    def update_ui_sizes(self, *args):
        ...
        self.subtitle_label.font_size = min(w * 0.038, dp(14))
        self.subtitle_label.size = (w, self.subtitle_label.font_size * 2)
        
        # Выделяем максимум места под список прокрутки
        self.scroll.height = h * 0.72

        # Геометрия современных строчных кнопок
        card_w = w - dp(40)
        card_h = dp(46)
        ...
        for card in self.menu_box.children:
            if isinstance(card, MenuCard):
                ...
                if hasattr(card, 'label_widget') and card.label_widget:
                    card.label_widget.font_size = sp(13)
...
class KrsBookApp(MDApp):      #Ядро. Запускает всё приложение, регистрирует экраны, настраивает тему и управляет системной кнопкой «Назад».
    ...
    def build(self):
        ...
        screens = [
            ...
            # Добавили экраны Блокнота в общее ядро
            NotebookMainMenu(name='notebook_main_menu'),
            CreateWellScreen(name='create_well_screen'),
            OpenWellScreen(name='open_well_screen'),
            ViewWellScreen(name='view_well_screen'),
            EditWellScreen(name='edit_well_screen')
        ]
...
class OpenWellScreen(MDScreen):
    ...
    def on_enter(self):
        ...
        self.ids.container_list.clear_widgets()
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if not rows:
            ...
            lbl = MDLabel(
                text="Журнал скважин пуст. Создайте запись.",
                halign="center",
                valign="middle",
            ...
            self.ids.container_list.add_widget(lbl)
        else:
            for row in rows:
                w_id = int(row[0])
                w_field = str(row[1])
    ...
    def open_view_screen(self, well_id):
        ...
        view_screen = self.manager.get_screen("view_well_screen")
        view_screen.current_well_id = well_id
        self.manager.current = "view_well_screen"
class EditWellScreen(MDScreen):
    ...
    def set_nkt_value(self, value):
        self.ids.e_nkt_drop.text = value
        if hasattr(self, 'menu') and self.menu:
            self.menu.dismiss()
    ...
    def on_enter(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        # Выгружаем все 17 столбцов, включая l_hvost на 9-м месте
        ...
        if w:
            self.ids.e_field.text = str(w[0]) if w[0] is not None else ""
            self.ids.e_pad.text = str(w[1]) if w[1] is not None else ""
            self.ids.e_well.text = str(w[2]) if w[2] is not None else ""
            self.ids.e_p_buf.text = str(w[3]) if w[3] is not None else ""
            self.ids.e_p_zatr.text = str(w[4]) if w[4] is not None else ""
            self.ids.e_p_pl.text = str(w[5]) if w[5] is not None else ""
            self.ids.e_rho_c.text = str(w[6]) if w[6] is not None else ""
            self.ids.e_depth.text = str(w[7]) if w[7] is not None else ""
            self.ids.e_l_hvost.text = str(w[8]) if w[8] is not None else ""  # Считываем длину хвостовика
            self.ids.e_perf_top.text = str(w[9]) if w[9] is not None else ""  # Индексы далее смещены на +1
            self.ids.e_perf_bot.text = str(w[10]) if w[10] is not None else ""
            self.ids.e_d_ek.text = str(w[11]) if w[11] is not None else ""
            self.ids.e_nkt_drop.text = str(w[12]) if w[12] else "Выбрать НКТ"
            self.ids.e_h_gno.text = str(w[13]) if w[13] is not None else ""
            self.ids.e_brigade.text = str(w[14]) if w[14] is not None else ""
            self.ids.e_operation.text = str(w[15]) if w[15] is not None else ""
            self.ids.e_notes.text = str(w[16]) if w[16] is not None else ""
    ...
    def update_well(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        # СТРОГИЙ ТЕХНОЛОГИЧЕСКИЙ ФИКС: Ровно 17 полей и ровно 17 знаков '?'
        cursor.execute(
            ...
            (self.ids.e_field.text, self.ids.e_pad.text, self.ids.e_well.text,
             self.ids.e_p_buf.text, self.ids.e_p_zatr.text, self.ids.e_p_pl.text, self.ids.e_rho_c.text,
             self.ids.e_depth.text, self.ids.e_l_hvost.text, self.ids.e_perf_top.text, self.ids.e_perf_bot.text,
             self.ids.e_d_ek.text, self.ids.e_nkt_drop.text, self.ids.e_h_gno.text,
             self.ids.e_brigade.text, self.ids.e_operation.text, self.ids.e_notes.text, self.current_well_id)
        )
        conn.commit()
        conn.close()
        self.go_back()
    ...
    def share_well(self):
        ...
        report = (
            ...
            f"💥 Давления: Рбуф={self.ids.e_p_buf.text}, Рзатр={self.ids.e_p_zatr.text}, Рпл={self.ids.e_p_pl.text} атм\n"
            ...
            f"📝 Заметки: {self.ids.e_notes.text}"
        )
        if platform == 'android':
    ...
    MDBoxLayout:
        ...
        ScrollView:
            MDBoxLayout:
                ...
                MDRaisedButton:
                    text: "🛢️   ОТКРЫТЬ В БЛОКЕ ГЛУШЕНИЯ"
...
class ViewWellScreen(MDScreen):
    ...
    def on_enter(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        # Выгружаем 16 параметров, включая rho_c строго на 7-м месте
        cursor.execute("SELECT field, pad, well, p_buf, p_zatr, p_pl, rho_c, depth, perf_top, perf_bot, d_ek, nkt_type, h_gno, brigade, operation, notes FROM wells WHERE id=?", (self.current_well_id,))
        w = cursor.fetchone()
        conn.close()
        ...
        if w:
            ...
            self.ids.v_rho_c.text = str(w[6]) if w[6] is not None else ""  # Считываем текущую плотность
            self.ids.v_depth.text = str(w[7]) if w[7] is not None else ""   # Индексы сдвинуты на +1
            self.ids.v_perf_top.text = str(w[8]) if w[8] is not None else ""
            self.ids.v_perf_bot.text = str(w[9]) if w[9] is not None else ""
            self.ids.v_d_ek.text = str(w[10]) if w[10] is not None else ""
            self.ids.v_nkt_type.text = str(w[11]) if w[11] is not None else ""
            self.ids.v_h_gno.text = str(w[12]) if w[12] is not None else ""
            self.ids.v_brigade.text = str(w[13]) if w[13] is not None else ""
            self.ids.v_operation.text = str(w[14]) if w[14] is not None else ""
            self.ids.v_notes.text = str(w[15]) if w[15] is not None else ""
    ...
    def share_well(self):
        ...
        report = (
            ...
            f"💥 Давления: Рбуф={self.ids.v_p_buf.text}, Рзатр={self.ids.v_p_zatr.text}, Рпл={self.ids.v_p_pl.text} атм\n"
    ...
    def open_in_glushenie(self):
        ...
        # Заполняем основные текстовые поля геометрии и давлений
        ...
        # СТРОГИЙ ТЕХНОЛОГИЧЕСКИЙ ФИКС: Обращаемся напрямую к переменной класса f_l_hvost в калькуляторе
